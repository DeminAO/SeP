...
namespace CrossMessaging.Client.CryptoService
{
	public class CryptoService : ICryptoService
	{
		...
		public async Task<string> EncryptStringAES(string plainText)
		{
			...
			try
			{
				...
				var sharedSecret = await GetSecureStringAsync();
		...
		private async Task<string> GetSecureStringAsync()
		{
			string secureCodeKey = "codeKey";
			try
			{
				var storageValue = await SecureStorage.GetAsync(secureCodeKey);
				if (string.IsNullOrWhiteSpace(storageValue))
				{
					storageValue = Guid.NewGuid().ToString();
					await SecureStorage.SetAsync(secureCodeKey, storageValue);
				}
			...
			{
				// secure key storage not supported
				throw;
			}
		}
	}
